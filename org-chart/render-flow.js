...
(async () => {
  ...
  await p.goto('file://' + __dirname + '/programme-flow.html');
  await p.waitForTimeout(400);
  const el = await p.$('.slide');
  await el.screenshot({ path: 'programme-flow.png' });
  const ov = await el.evaluate(e => ({ sh: e.scrollHeight, ch: e.clientHeight }));
  if (ov.sh > ov.ch + 2) console.log('  ! overflow', ov);
  await p.pdf({ path: 'programme-flow.pdf', width: '1600px', height: '900px',
